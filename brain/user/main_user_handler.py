...
from body.user.modules.faq.user_m_f_keyboard import faq_inline_keyboard
...
def main_user_handler(vk, event):
    ...
    # Если запрашивается любая из услуг
        ...
        vk.messages.send(
            user_id=event.user_id,
            message='Если ты не найдешь ответа на свой вопрос, ты всегда можешь '
                    'оставить нам заявку.',
            random_id=random_id(),
            keyboard=faq_inline_keyboard()
